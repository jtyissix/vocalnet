...
from .configuration_whisper import WhisperStreamConfig
...
class WhisperStreamEncoderLayer(nn.Module):
    def __init__(self, config: WhisperStreamConfig, is_causal=False):
        super().__init__()
...
class WhisperDecoderLayer(nn.Module):
    def __init__(self, config: WhisperStreamConfig, layer_idx: int = None):
        super().__init__()
        self.embed_dim = config.d_model
...
class WhisperPreTrainedModel(PreTrainedModel):
    config_class = WhisperStreamConfig
    ...
    def _init_weights(self, module):
        ...
        elif isinstance(module, WhisperStreamEncoder):
            with torch.no_grad():
...
class WhisperStreamEncoder(WhisperPreTrainedModel):
    """
    Transformer encoder consisting of *config.encoder_layers* self attention layers. Each layer is a
    [`WhisperStreamEncoderLayer`].
    ...
    def __init__(self, config: WhisperStreamConfig):
        ...
        self.layers = nn.ModuleList([WhisperStreamEncoderLayer(config,
        ...
        # else:
        #     self.layers = nn.ModuleList([WhisperStreamEncoderLayer(config, is_causal=config.encoder_causal_attention or (
...
class WhisperStreamDecoder(WhisperPreTrainedModel):
    """
...
class WhisperStreamModel(WhisperPreTrainedModel):
    def __init__(self, config: WhisperStreamConfig):
        super().__init__(config)

        self.encoder = WhisperStreamEncoder(config)
        self.decoder = WhisperStreamDecoder(config)
        # Initialize weights and apply final processing
        self.post_init()
    ...
    def forward(
        ...
        Example:
         ...
         >>> model = WhisperStreamModel.from_pretrained("openai/whisper-base")
...
class WhisperStreamForConditionalGeneration(WhisperGenerationMixin, WhisperPreTrainedModel):
    base_model_prefix = "model"
    _tied_weights_keys = ["proj_out.weight"]

    def __init__(self, config: WhisperStreamConfig):
        super().__init__(config)
        self.model = WhisperStreamModel(config)
    ...
    def forward(
        ...
        >>> model = WhisperStreamForConditionalGeneration.from_pretrained("openai/whisper-tiny.en")
...
class WhisperForCausalLM(WhisperPreTrainedModel):
    ...
    def forward(
        ...
        >>> model = WhisperStreamForConditionalGeneration.from_pretrained("openai/whisper-large-v2")
