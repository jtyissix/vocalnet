...
class CosyVoice2(CosyVoice):
    ...
    def inference_zero_shot_ttst(self, tts_text, prompt_text, prompt_speech_16k, stream=False, speed=1.0):
        prompt_text = self.frontend.text_normalize(prompt_text, split=False)
        for i in tqdm(self.frontend.text_normalize(tts_text, split=True)):
            if len(i) < 0.5 * len(prompt_text):
                logging.warning('synthesis text {} too short than prompt text {}, this may lead to bad performance'.format(i, prompt_text))
            ...
            # import pdb
            # pdb.set_trace()
            for model_output in self.model.text_to_speech_token(**model_input, stream=stream, speed=speed):
                speech_len = len(model_output['speech_token'])
                # pdb.set_trace()
                logging.info('yield speech len {}, rtf {}'.format(speech_len, (time.time() - start_time)))
                yield model_output
                start_time = time.time()
